...
impl Element {
    pub fn new(ns: (Rc<Node>, Rc<Node>), props: ElementProperties, dim: usize) -> Element {
        Element {
            nodes: ns,
            properties: props,
            local_k: Matrix::zeros(dim, dim),
            global_k: Matrix::zeros(dim, dim),
            t_matrix: Matrix::zeros(dim, dim),
            load: 0.0,
            load_pos: 0.0,
            load_type: LoadType::PointLoad,
            p_local: Matrix::zeros(6, 1),
            p_global: Matrix::zeros(6, 1),
        }
    }

    pub fn get_element_dof(&self) -> ((usize, usize, usize), (usize, usize, usize)) {
    ...
    pub fn find_global_k(&mut self) {
        let t_trans = self.t_matrix.transpose();
    ...
    pub fn calculate_fes(&mut self) {
        ...
        self.p_global = self.t_matrix.transpose() * &self.p_local;
    }
}
